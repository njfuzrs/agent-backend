...
`test_boundaries.py`、`test_identity.py`、`test_flag.py`、`test_policy.py`、`test_event.py`、`test_cost.py`、`test_deploy_scripts.py` 进 CI。本目录其余脚本对着**活服务端**跑，不进 CI。
...
| `test_event.py` | `POST /events` 401 / 幂等 / 白名单 / session-coverage / policy-audit | 无活服务端（独立 SQLite） |
| `test_cost.py` | `POST /usage/ledger` 401 / 30 次上报 1 行 / `GET /ctl/budget` device>team>org / 默认 alert / by-scope | 无活服务端（独立 SQLite） |
