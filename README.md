...
sid-code 与 claude-trace 共同面对的企业级后端：控制面（policy / flag / 身份）与数据面（轨迹 / 事件）同仓部署、鉴权隔离。当前已交付轨迹存储与分析模块，其余模块按里程碑迭代。
...
- 双平面鉴权骨架：数据面走 `X-Upload-Token` / Basic Auth / 管理台 cookie；控制面走 `require_device`，两条链互不引用

**没有（路线图，不要当成已交付）**

- 设备身份签发（`require_device` 目前对 `/ctl/` **501**）
- Flag 下发
- 远程 Policy
- 组织事件接收
- 成本账本对账
- 生产 TLS / 登录体系（管理台目前是共享口令 + cookie）
...
| M1 | 设备身份 / `require_device` 从 501 变成真鉴权 | 未开工 |
| M2 | Flag 下发 | 未开工 |
